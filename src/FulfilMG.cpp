...
{ int i,j,f,k; int max, i_max, non_i;
  ...
  {   max=0;
	  ...
	  for(j=0;j<n;j++)
	  { if(non[j]==-1){
	    if(Matrix().num[j].vec[i]*Matrix().num[j].vec[i]>max)
	    { max=Matrix().num[j].vec[i]*Matrix().num[j].vec[i];
	      i_max=j;}
	    }
	  }
	  non[i_max]=0;
	  
      if(Matrix().num[i_max].vec[i]!=0)
	  { for(f=0;f<n;f++)
	    { if(f!=i_max){
		  con1=Matrix().num[i_max].vec[i];
		  if(Matrix().num[f].vec[i]!=0)
	      { con1=Matrix().num[f].vec[i]/con1;
	        Matrix().num[f]=Matrix().num[f]+(Matrix().num[i_max]*con1*(-1));
			Vector_B().vec[f]= Vector_B().vec[f]- Vector_B().vec[i_max]*con1;
		  } }
	    }
	  } std::cout<<"\n\n\n"<<Matrix();
	  std::cout<<"\n\n\n"<<Vector_B();
...
void MG::DEC()//решение Метод Гаусса
{
	...
	if((flag+1)/2*n*n<550){//std::cout<<"1\n";
	DEC1();} else{
	if((flag+1)*(flag+1)+n*n>10100){//std::cout<<"3\n";
	DEC3();} else {//std::cout<<"2\n";
	DEC2();}}
}
